...
void EposMotor::init(ros::NodeHandle &root_nh, ros::NodeHandle &motor_nh, const std::string &motor_name)
{
    ...
    m_state_subscriber = motor_nh.subscribe("set_state", 1, &EposMotor::writeCallback, this);
}
...
void EposMotor::initEposDeviceHandle(ros::NodeHandle &motor_nh)
{
    ...
    ROS_INFO_STREAM("Node id: " << node_id);

    ROS_INFO_STREAM("Device Info Protocol_stack: " << device_info.protocol_stack.c_str());
    // create epos handle
...
void EposMotor::initProfilePosition(ros::NodeHandle &motor_nh)
{
    ...
    if (profile_position_nh.hasParam("velocity")) {
        ...
        ROS_INFO_STREAM("Set to ProfilePositionMode!");
    }
}
...
void EposMotor::initProfileVelocity(ros::NodeHandle &motor_nh)
{
    ...
    if (profile_velocity_nh.hasParam("acceleration")) {
        ...
        ROS_INFO_STREAM("Set to Velocity Profile!");
    }
}

// void EposMotor::initHoming(ros::NodeHandle &motor_nh)
// {
//     VCS_NODE_COMMAND_NO_ARGS(ActivateHomingMode, m_epos_handle);
//     ROS_INFO_STREAM("Set to Homing!");
// }
/**
...
// void EposMotor::homingAttain() {
//     VCS_COMMAND(WaitForHomingAttained, m_epos_handle, 1000)
// }

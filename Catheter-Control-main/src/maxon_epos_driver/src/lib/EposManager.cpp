        ...
        const std::vector<std::string> &motor_names)
{
    ...
    m_all_motor_subscriber = motors_nh.subscribe("set_all_states", 1, &EposManager::write, this);
    return true;
}
...
void EposManager::write(const maxon_epos_msgs::MotorStates::ConstPtr& msg)
{
    for (int i = 0; i < m_motors.size(); i++) {
        ...
        ROS_INFO_STREAM("Send pos: " << state.motor_name << " " << state.position);
        // ROS_INFO_STREAM("Send vel: " << state.motor_name << " " << state.velocity);

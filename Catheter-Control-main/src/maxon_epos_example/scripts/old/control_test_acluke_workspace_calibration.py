#!/usr/bin/python3
# Last Edited: 18Mar2023 by Alexander Luke
...
EXISTING_CALIBRATION = 0                    # SET TO 1 if there is an existing file with calibration parameters you want to use, otherwise 0
CATHETER_ID = 'AL04062301'                  # identification # for a given catheter; format: (initials of assembler)(MM)(DD)(YY)(unit ## made that day)
CALIBRATION_ID = '18Apr2023'                # date of calibration file to be saved/read
...
class CatheterController():
    ...
    def tensioningSequence(self):
        ...
        if EXISTING_CALIBRATION == 1:
            data_capture_file_name = 'Calibration_'+CALIBRATION_ID+'_'+CATHETER_ID+'.npy'
            cali_zero_angles = np.load('/home/arclab/'+data_capture_file_name)
            print('! ! ! - An existing calibration file has been found - ! ! !')
            print('Found offsets: ',cali_zero_angles)
            print(' ')
            ...
            target_position = cali_zero_angles
            ...
            print("   Once motion is seen in the catheter, proceed to the next step by pressing the 'w' key")

            num_tension_attempts        = 3
            cali_zero_angles            = np.zeros(4)   # in rotations, not radians or degrees. STARTS AS ZEROS
            cali_step_scale_factor      = 0.5
            cali_release_scale_factor   = 0.5
            displacement_limit          = 0.5           # in rotations
            rotational_step_size        = 0.01          # in rotations

            for j in range(num_tension_attempts):

                print(' ')
                step = rotational_step_size*(cali_step_scale_factor**j)
                print('Attempt '+str(j+1)+' has begun with step size of '+str(round(step,5)))
                print('cali_zero_angles: ',cali_zero_angles)

                for i in range(0,MOTOR_NUM):

                    tw_start_angle = target_position[i]
                    print("      Tensioning Spool "+str(i+1)+" ... Start: "+str(tw_start_angle))
                    ...
                    while(True):
                        ...
                        if(a=='w'):
                            ...
                            print('Tensioned Position: ',target_position[i])
                            delta = target_position[i]-tw_start_angle
                            print('Delta: '+str(round(delta,5))+' of a rotation')
                            print('cali_zero_angles: ',cali_zero_angles,' target_position: ',target_position)

                            cali_zero_angles[i] = cali_zero_angles[i]+target_position[i]
                            print('Current summed calibration vector',cali_zero_angles)

                            if delta==0:
                                target_position[i] = target_position[i]-rotational_step_size
                                
                            else:
                                # Release the tendon wire by a small amount
                                target_position[i] = delta*cali_release_scale_factor

                            self.setAllTargetPositions(target_position)
                            CCTRL.desired_motor_state_pub.publish(CCTRL.desired_motor_state)
                            print('Motor released to ',target_position[i])

                            print('            [W] key pressed. Moving to next step.')
                            break

                        if(a=='p' and -displacement_limit<=target_position[i]+step<=displacement_limit):

                            target_position[i] = target_position[i]+step
                            print('            + [P] key pressed. New Target: ',target_position)
                            self.setAllTargetPositions(target_position)
                            CCTRL.desired_motor_state_pub.publish(CCTRL.desired_motor_state) 

                        if(a=='m' and -displacement_limit<=target_position[i]-step<=displacement_limit):
            ...
            cali_zero_angles = cali_zero_angles/num_tension_attempts
            print('Calibration vector after averaging: ',cali_zero_angles)
            ...
            data_capture_file_name = 'Calibration_'+CALIBRATION_ID+'_'+CATHETER_ID
            np.save(data_capture_file_name,cali_zero_angles)
            print(' ')
            print('The calibration has been saved as '+data_capture_file_name+'.npy')

        return cali_zero_angles
    ...
    def workspaceCalibration(self):
        ...
        # Check if calibration is necessary
        if EXISTING_CALIBRATION == 1:
            data_capture_file_name = 'WorkspaceCalibration_'+CALIBRATION_ID+'_'+CATHETER_ID+'.npy'
            cali_zero_angles = np.load('/home/arclab/'+data_capture_file_name)
            print('! ! ! - An existing calibration file has been found - ! ! !')
            print('Found offsets: ',cali_zero_angles)
            print(' ')
            ...
            num_attempts                = 3
            data_capture                = []                    # [Angle  Tension_Wire  Tensioned_Position  Detension_Wire  Detensioned_Position]
            cali_step_scale_factor      = 0.5
            cali_release_scale_factor   = 0.5
            displacement_limit          = 0.5                   # in rotations
            rotational_step_size        = 0.01                  # in rotations
            workspace_angles            = np.array([0,22.5,45,67.5,90])   # the distal tip angles we want to calibrate at
            ...
            for j in range(num_attempts):
                ...
                for current_pair in wire_pairs:
                    ...
                    # Release tendon wires to startup zero position 
                    target_position = np.zeros(4)
                    CCTRL.setAllTargetPositions(target_position)
                    CCTRL.desired_motor_state_pub.publish(CCTRL.desired_motor_state)    
                    rospy.sleep(1)
                    ...
                    for angle in workspace_angles:
                        ...
                        while(True): 
                            ...
                            if(a=='w'):
                                ...
                                print('Tensioned Delta: '+str(round(delta_tw,5))+' of a rotation ; Detensioned Delta: '+str(round(delta_dw,5))+' of a rotation')

                                data_vector  = np.array([angle,
                                                         tension_wire,
                            ...
                            if(a=='P' and -displacement_limit<=target_position[detension_wire]+step<=displacement_limit):
                                ...
                                print('            + [Shift+P] key pressed. New Target: ',target_position)
                                self.setAllTargetPositions(target_position)
                                CCTRL.desired_motor_state_pub.publish(CCTRL.desired_motor_state) 
                            ...
                            if(a=='M' and -displacement_limit<=target_position[detension_wire]-step<=displacement_limit):
                                ...
                                target_position[detension_wire] = target_position[detension_wire]-step
                                print('            - [Shift+M] key pressed. New Target: ',target_position)
                                self.setAllTargetPositions(target_position)
                                CCTRL.desired_motor_state_pub.publish(CCTRL.desired_motor_state)

                        rospy.sleep(0.1)  

            data_capture_file_name = 'WorkspaceDataCapture_'+CALIBRATION_ID+'_'+CATHETER_ID
...
if __name__ == "__main__": 
    ...
    tension_offsets = CCTRL.workspaceCalibration()
